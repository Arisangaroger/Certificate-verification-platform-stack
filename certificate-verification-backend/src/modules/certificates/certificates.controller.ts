...
import { IssueToBlockchainDto } from './dto/issue-to-blockchain.dto';
...
export class CertificatesController {
  ...
  @ApiOperation({ summary: 'List certificates for a student' })
  ...
  @Get('stats')
  @ApiOperation({ summary: 'Global certificate statistics' })
  @ApiOkResponse({ type: CertificateStatsDto })
  async getStats() {
    return this.certificatesService.getStats();
  }

  @Get('university/:universityId')
  @ApiOperation({ summary: 'List certificates by university' })
  ...
  @ApiOperation({ summary: 'Certificate statistics by university' })
  @ApiParam({ name: 'universityId', format: 'uuid' })
  @ApiOkResponse({ type: CertificateStatsDto })
  ...
  @ApiOperation({
    summary: 'Issue certificates to blockchain',
    description:
      'Batch mint selected certificate hashes on Optimism Sepolia via CredentialRegistry.issueCredentials. Updates status to VERIFIED and stores the transaction hash.',
  })
  @ApiParam({ name: 'universityId', format: 'uuid' })
  @ApiOkResponse({ type: IssueToBlockchainResponseDto })
  @ApiResponse({ status: 400, description: 'Invalid batch or blockchain error' })
  @ApiResponse({ status: 404, description: 'Certificates not found' })
  async issueToBlockchain(
    @Param('universityId') universityId: string,
    @Body() body: IssueToBlockchainDto
  ) {
    // 1. Validate input
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Update certificate', description: 'Only allowed before blockchain issuance.' })
  @ApiParam({ name: 'certificateId', example: 'Vpg3YCjx7i1O' })
  async updateCertificate(
    @Param('certificateId') certificateId: string,
    @Body() updateData: UpdateCertificateDto
  ) {
    try {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Delete certificate', description: 'Only allowed before blockchain issuance.' })
  @ApiParam({ name: 'certificateId', example: 'Vpg3YCjx7i1O' })
